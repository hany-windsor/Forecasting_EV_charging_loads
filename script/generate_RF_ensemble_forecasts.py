...
import re
import json
from script import RF_ensemble_forecast, adding_context_to_basic_forecasters
...
for input_file_name in list_of_dates:
    ...
    combined_ensemble_data = pd.concat(
        [ensemble_data_training_with_context, ensemble_data_testing_with_context],
        axis=0,
        ignore_index=True
    )

    # Step 3: Save to CSV
    combined_ensemble_data.to_csv(f"../data/ensemble_data_with_context_{date_range}.csv", index=False)
    ...
    training_time_of_one_model= RF_ensemble_forecast.RF_ensemble(date_range, ensemble_composition, ensemble_data_training_with_context, ensemble_data_testing_with_context)
    training_time[ensemble_composition][date_range]= training_time_of_one_model


# Save to JSON file
with open(f"../forecasting_results/random_forest_results/training_time.json", "w") as json_file:
